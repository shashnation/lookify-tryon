export default async function handler(req, res) {
  ...
  try {
    const { customerPhotoBase64, garmentImageUrl, category } = req.body || {};

    if (!customerPhotoBase64) {
      return res.status(400).json({ error: "Customer photo is required." });
    }
    ...
    const response = await fetch("https://corlen.io/api/v1/tryon", {
      method: "POST",
      headers: {
